...
const stack = () => {
  const text = ERROR_STRING().split(/\n+/g).map(v => v.trim()).filter(v => BASE_STACK.indexOf(v) === -1);
  return text;
};

if (!BASE_STACK.length) BASE_STACK = stack();
export const stacktrace = verbose => {
  ...
  if (verbose === 1) {
    text = text.map(v => v.split(/:/g).slice(-2).join(':').replace(/[^\d:]/g, ''));
