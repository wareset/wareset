...
function smartsplit(str, d = ',', q = '\'"`') {
  ...
  for (let i = 0; i < columnsDirty.length; i++) {
    ...
    if (match.length > 1) {
      match = match.map(v => v[v.length - 1]).filter((v, k, a) => isQuote ? v === a[a.length - 1] : v === a[0]);
    }

    isMatch = !!(match.length % 2);
    if (isMatch !== (isMatch === isQuote)) res.push(column);else res[res.length - 1] += delimeters[i - 1] + column;
